...
import { QuizNotFoundIcon, QuizNotPublishedIcon } from "@/icons";
...
export const QuizRenderer: React.FC<QuizRendererProps> = ({ quiz }) => {
  if (!quiz) {
    return (
      <div className="max-w-2xl mx-auto p-8 text-center bg-white rounded-lg shadow-sm border border-gray-200">
        <QuizNotFoundIcon className="w-16 h-16 mx-auto mb-4 text-gray-400" />
  ...
  if (!quiz.published) {
    return (
      <div className="max-w-2xl mx-auto p-8 text-center bg-white rounded-lg shadow-sm border border-gray-200">
        <QuizNotPublishedIcon className="w-16 h-16 mx-auto mb-4 text-red-400" />
        <h1 className="text-2xl font-bold text-gray-900 mb-4">
          Quiz Not Published
        </h1>
        <p className="text-gray-600 mb-6">
            ...
            switch (block.type) {
              ...
              case BlockType.Question:
                return (
                  <QuestionBlock
                    key={block.id}
                    id={block.id}
                    {...(block.properties as QuestionProperties)}
                  />
                );
              case BlockType.Button:
                return (
                  <ButtonBlock
                    key={block.id}
                    {...(block.properties as ButtonProperties)}
                  />
                );
              case BlockType.Footer:
                return (
                  <FooterBlock
                    key={block.id}
                    {...(block.properties as FooterProperties)}
                  />
                );
              default:
                return null;
