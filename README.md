...
1. Insert-Before DnD Feature
   - Currently, the canvas supports drag-and-drop reordering of blocks and adding new blocks at the end.
   - There is no separate droppable area for inserting a new block *before* a specific existing block.
   - Users can only reorder blocks or append new ones at the end.
2. Drag-and-drop Performance: Occasional lag with large quizzes
3. No Optimistic Updates: UI may briefly freeze during operations
4. Limited Error Handling: Some errors not handled gracefully
5. Missing Loading States: Some operations lack visual feedback
6. Application is not usable on screens with resolution less than 768px
